				...
				if ($result) {
					if ($result->status_code == 200) {
						...
						<div class="popup-content cointopay active">    
							<img src="'.$this->_assetRepo->getUrlWithParams('Cointopay_PaymentGateway::images/cointopay.gif', $params).'" />
							...
							if (@fopen('https://quickchart.io/qr?size=300&text='.$response->coinAddress, 'r')) {
												...
												$.ajax ({
													...
													success: function(result) {
														if (result.status == \'paid\') {
															$(\'.popup-content.cointopay\').html(\'<h3>Thank you, your payment is received!</h3><p class="description"> <a target="_blank" href="'. $response->RedirectURL .'">View Invoice details</a></p>\');
														} else if (result.status == \'expired\') {
															$(\'.popup-content.cointopay\').html(\'<h3>Payment time expired!</h3><p class="description"> <a target="_blank" href="'.  $response->RedirectURL .'">View Invoice details</a></p>\');
														}
														 else if (result.status == \'underpaid\') {
															$(\'.popup-content.cointopay\').html(\'<h3>You have underpaid, please pay the remaining amount or contact support!</h3><p class="description"> <a target="_blank" href="'.  $response->RedirectURL .'">View Invoice details</a></p>\');
														}
													}
												});
											}
